<template>
    <el-dialog title="修改" width="70%"  :visible.sync="param.visible" @close="param.close" :close-on-click-modal="false">
        ...
        save(){
            let flag = this.$refs['form'].validate();
                if(flag){
                    ...
                    request({url:'wikipedia/wikipedia/'+this.form.id+'/',method:'put',data}).then(res=>{
                        this.$message({
                              message: '修改成功！！！',
                              type: 'success'
                            });
